...
sub NEWMETADATA {
    ...
    $theFormat = Foswiki::Func::expandTemplate($theTemplate)
...
sub renderMetaData {
    ...
    $theMandatory = " <span class='foswikiAlert'>**</span> "
      unless defined $theMandatory;
    $theHiddenFormat = '<input type="hidden" name="$name" value="$value" />'
      unless defined $theHiddenFormat;

    my $metaDataKey = uc($metaData);
    my $metaDataDef = $Foswiki::Meta::VALIDATE{$metaDataKey};
    return inlineError("meta data $metaDataKey not found")
      unless defined $metaDataDef;

    my $formWeb   = $this->{baseWeb};
    my $formTopic = $metaDataDef->{form};

    $formTopic =
      $Foswiki::cfg{SystemWebName} . '.' . ucfirst( lc($metaData) ) . 'Form'
 ...
 # }
    ...
    ( $formWeb, $formTopic ) =
      Foswiki::Func::normalizeWebTopicName( $formWeb, $formTopic );

    #writeDebug("formWeb=$formWeb, formTopic=$formTopic");

    unless ( Foswiki::Func::topicExists( $formWeb, $formTopic ) ) {
        print STDERR
          "error: form definition for $formWeb.$formTopic does not exist";
        return inlineError(
            "form definition for $formWeb.$formTopic does not exist");
    }

    my $formDef = new Foswiki::Form( $this->{session}, $formWeb, $formTopic );
    unless ( defined $formDef ) {
        print STDERR
          "error: can't parse form definition at $formWeb.$formTopic";
        return inlineError(
            "can't parse form definition at $formWeb.$formTopic");
    }
    ...
    unless ( defined $theHeader ) {
        if ( $theAction eq 'view' ) {
            $theHeader = '<div class=\'metaDataView\'>$n| *' . join(
                ...
                map {
                    my $title = $_->{title};
                    defined( $params->{ $title . '_title' } )
                      ? $params->{ $title . '_title' }
                      : $title
                  }
                  grep { $_->{name} ne 'name' } @selectedFields
            ) . '* |$n';
        }
        else {
    ...
    unless ( defined $theFormat ) {
        if ( $theAction eq 'view' ) {
            ...
            $theFormat =
'<div class=\'foswikiFormStep $metadata\'>$n<table class=\'foswikiLayoutTable\'>$n'
    ...
    foreach my $record ( sort { $a->{name} cmp $b->{name} } @metaDataRecords ) {
        ...
        foreach my $field (@selectedFields) {
            ...
            if ( $theAction eq 'edit' ) {
                $field->{name} =
                  'META:' . $metaDataKey . ':' . $name . ':' . $fieldName;
            }
...
# CAUTION: don't use field->getOptions() on a +values field as that won't return the full valueMap...only the value part, but not the title map
            if ( $field->can('getOptions') && $field->{type} !~ /\+values/ ) {
                ...
                if ($options) {
                    $fieldAllowedValues =
                      join( $theValueSep, split( /\s*,\s*/, $options ) );
                }
            }
        ...
        # get the list of all allowed values without any +values mapping applied
            ...
            $fieldSize = $params->{ $fieldName . '_size' }
              if defined $params->{ $fieldName . '_size' };
            $fieldAttrs = $params->{ $fieldName . '_attributes' }
              if defined $params->{ $fieldName . '_attributes' };
            $fieldDescription = $params->{ $fieldName . '_tooltip' }
              if defined $params->{ $fieldName . '_tooltip' };
            $fieldDescription = $params->{ $fieldName . '_description' }
              if defined $params->{ $fieldName . '_description' };
            $fieldTitle = $params->{ $fieldName . '_title' }
              if defined $params->{ $fieldName . '_title' };
            $fieldAllowedValues = $params->{ $fieldName . '_values' }
              if defined $params->{ $fieldName . '_values' };
            $fieldType = $params->{ $fieldName . '_type' }
              if defined $params->{ $fieldName . '_type' };
            $fieldValue = $params->{ $fieldName . '_value' }
              if
                defined $params->{ $fieldName
                        . '_value' };    # or get value from macro invocation
            $fieldFormat = $params->{ $fieldName . '_format' }
              if defined $params->{ $fieldName . '_format' };
            $fieldDefault = $params->{ $fieldName . '_default' }
              if defined $params->{ $fieldName . '_default' };

            my $fieldIsHidden =
              Foswiki::Func::isTrue( $params->{ $fieldName . '_hidden' }, 0 );
            ...
            if ( $theAction eq 'edit' ) { # or get value from url (highest prio)
                ...
                if ( $field->isMultiValued ) {
                    my @urlValue =
                      $query->param( 'META:' . $metaData . ':' . $fieldName );
            ...
            my $fieldAutolink =
              Foswiki::Func::isTrue( $params->{ $fieldName . '_autolink' },
                $theAutolink );
            my $fieldSort =
              Foswiki::Func::isTrue( $params->{ $fieldName . '_sort' } );
...
#writeDebug("metaData=$metaData, fieldName=$fieldName, fieldValue=$fieldValue");
            ...
            $line = '<noautolink>' . $line . '</noautolink>'
              unless $fieldAutolink;
            ...
            unless ($fieldIsHidden) {
                $fieldValue = "\0"
                  unless
                    defined $fieldValue;  # prevent dropped value attr in CGI.pm
                ...
                if ( $theAction eq 'edit' ) {
                    if ( $Foswiki::Plugins::VERSION > 2.0 ) {
                        ...
                        # pre-TOM
                        ( $fieldExtra, $fieldEdit ) =
                          $field->renderForEdit( $topicObj->web,
                            $topicObj->topic, $fieldValue );
                    }
                }
                else {
      ...
      /ge;
        ...
        my $fieldDeleteAction =
          Foswiki::Func::expandTemplate("metadata::delete");
        my $fieldDuplicateAction =
          Foswiki::Func::expandTemplate("metadata::duplicate");
        $fieldDuplicateAction = '';    # TODO: disabled

        my $fieldActions =
            '<span class="metaDataActions">'
          . $fieldEditAction
...
sub getKnownMetaData {
    ...
    unless ( defined $this->{_knownMetaData} ) {
        ...
        foreach my $name ( sort keys %Foswiki::Meta::VALIDATE ) {
            next
              if $name =~
/^(TOPICINFO|VERSIONS|CREATEINFO|STORAGE|TOPICMOVED|FIELD|FORM|FILEATTACHMENT|TOPICPARENT)$/;
    ...
    foreach my $urlParam ( $request->param() ) {
        ...
        unless ( defined $record ) {
            ...
            if ( $name eq 'id' ) {
                ...
                $record =
                  { name => 'id'
                      . ( $this->getMaxId( $metaDataName, $meta ) + 1 ), };
            }
            else {
...
sub jsonRpcDelete {
    ...
    throw Foswiki::Contrib::JsonRpcContrib::Error( 404,
        "Topic $web.$topic does not exist" )
      unless Foswiki::Func::topicExists( $web, $topic );

    throw Foswiki::Contrib::JsonRpcContrib::Error( 401, "Access denied" )
      unless Foswiki::Func::checkAccessPermission( "CHANGE", $wikiName, undef,
        $web, $topic );
    ...
    throw Foswiki::Contrib::JsonRpcContrib::Error( 1000,
        "unknown meta $metaData" )
      unless defined $Foswiki::Meta::VALIDATE{$metaDataKey};

    my $record = $meta->get( $metaDataKey, $name );

    throw Foswiki::Contrib::JsonRpcContrib::Error( 1001,
        "$metaData name=$name not found" )
      unless $record;

    writeDebug( "$this, checking deleteHandler for $metaDataKey ... "
          . $this->{deleteHandler}{$metaDataKey} );
    if ( defined $this->{deleteHandler}{$metaDataKey} ) {
        foreach my $deleteHandler ( @{ $this->{deleteHandler}{$metaDataKey} } )
        {
            ...
            print STDERR "error executing deleteHandler $function: " 
              . $error . "\n"
              if defined $error;
        }
    }
